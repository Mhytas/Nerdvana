...
module.exports = async (bot, interaction) => {
    ...
    await db.query(`SELECT * FROM config_ticket WHERE guildID = '${interaction.guild.id}'`, async (err, req) => {
        ...
        //salon
        if(interaction.customId === "selectmenu_config_ticket_salon" || interaction.customId === "selectmenu_config_ticket_salon_messsage" || interaction.customId === "selectmenu_config_ticket_salon_embed") {
            const selectedCommand = interaction.values[0];
            db.query(`UPDATE config_ticket SET salon = '${selectedCommand}' WHERE guildID = '${interaction.guild.id}'`)
            ...
            if(interaction.customId === "selectmenu_config_ticket_salon") await interaction.deferUpdate()
            if(interaction.customId === "selectmenu_config_ticket_salon") await interaction.editReply({embeds: [embed_config_ticket], components: [actionRow, btns, btns2], ephemeral: true})
        }


        //logs
        if(interaction.customId === "selectmenu_config_ticket_logs") {
            const selectedCommand = interaction.values[0];
            db.query(`UPDATE config_ticket SET logs_ticket = '${selectedCommand}' WHERE guildID = '${interaction.guild.id}'`)
