...
module.exports = async(bot, member, type, invite) => {
    ...
    await db.query(`SELECT * FROM server WHERE guild = '${member.guild.id}'`, async (err, req) => {
        ...
        await db.query(`SELECT * FROM server WHERE guild = '${member.guild.id}'`, async (err, all) => {
            ...
            const welcomeChannel = await member.guild.channels.cache.find((channel) => channel.id === all[0].arrive);

            const canvas = Canvas.createCanvas(700, 250);
            const ctx = canvas.getContext('2d');
        
            // Load background image
            const background = await Canvas.loadImage("https://cdn.discordapp.com/attachments/1021099920356278324/1244385577546027038/leaderboard_black.png?ex=665594a1&is=66544321&hm=8aa1c554abd019f25ee2af23e193ac28461eea176879b0e04387bbf1ed3edbe6&");
            ...
            return await welcomeChannel.send({files: [new Discord.AttachmentBuilder(canvas.toBuffer(), {name: "home.png"})]})
