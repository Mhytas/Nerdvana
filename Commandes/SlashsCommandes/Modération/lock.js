...
module.exports = {
    ...
    utilisation: "/lock [raison] (role) (salon)",
    type: 1,
    ...
    async run(bot, message, args, db) {
        ...
        message.reply({ content: `Le rôle **\`${role.name}\`** a été **LOCK** dans le salon **${channel}** pour la raison **\`${reason}\`** !`, ephemeral: true })
        channel.send(`Ce salon a été **LOCK** pour le rôle **\`${role.name}\`** par <@${message.user.id}> pour la raison **\`${reason}\`** !`)
    }
}